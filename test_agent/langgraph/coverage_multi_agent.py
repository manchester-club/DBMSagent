...
**标准工作流程**：
1) 用 Collect_Coverage 收集覆盖率；2) 用 Search_Nearest_Seed 查找相关 SQL 种子；3) 用 Code Explorer 获取其源代码；4) 用 Traverse_Call_Graph 向上遍历调用关系；5) 用 Coverage Analyzer 生成 SQL 测试用例；6) 用 SQL Tester 执行 SQL 测试用例；7) 用 Collect_Coverage 验证覆盖率是否提升。8) 如果覆盖率没有提升，重复步骤 2-7。9) 如果覆盖率提升，结束。
...
SUMMARY_PROMPT = """你正在生成一份**详细、完整、结构化**的测试报告，重点关注如何通过 SQL 测试用例提高代码覆盖率。

**⚠️ 关键要求**：
1. **必须生成完整的 Markdown 格式报告**，不要只输出工具调用或简单文本
2. **必须提取所有实际数据**：从工具执行结果中提取具体的数值、代码、SQL 语句、行号等
3. **必须包含代码片段**：展示实际的源代码，不要只说"有分支"
4. **必须包含 SQL 语句**：展示完整的 SQL 代码，不要只说"生成了 SQL"
5. **必须详细分析**：说明分析过程和依据，不要只写结论
6. **必须完整准确**：不要遗漏重要信息，不要猜测或简化
7. **报告长度**：报告应该足够详细，至少 2000 字以上，包含所有关键信息的具体内容

**⚠️ 禁止行为**：
- ❌ 不要只输出工具调用格式（如 <｜DSML｜function_calls>）
- ❌ 不要只写一句话总结
- ❌ 不要省略具体数据（覆盖率、行号、SQL 语句等）
- ❌ 不要使用占位符（如 [从工具结果中提取]），必须填入实际数据
...
**必须从 Collect_Coverage 或 Get_Code_Context 工具结果中提取以下信息**：

- **目标函数**：[实际函数名，如 `datan2`]
- **文件路径**：[完整路径，如 `postgresql-17.6/src/backend/utils/adt/float.c`]
- **行号**：[实际行号，如 `1850`]
- **函数签名**：[从 Get_Code_Context 结果中提取完整函数签名]
- **测试目的**：提高该函数的代码覆盖率，特别是覆盖标记为 `#####` 的未覆盖行

## 2. 源代码分析（详细）

**必须从 Get_Code_Context 工具结果中提取并展示**：

- **函数签名**：[完整签名，包括参数类型和返回类型，从 `source_code` 字段提取]
- **完整源代码**：[展示完整的函数源代码，使用代码块格式]
- **核心逻辑**：[详细说明函数的主要功能和处理流程，至少 200 字]
- **关键代码片段**：[**必须包含实际的代码**，特别是包含分支判断的关键部分，使用代码块格式]
- **分支结构**：[**详细列出所有 if/else、switch 等分支**，包括：
  - 每个分支的条件（展示实际代码）
  - 每个分支的行号（从 gcov_content 中提取）
  - 每个分支的功能说明（至少 50 字）
  - 哪些分支已覆盖，哪些未覆盖（标记为 `#####`）]
- **未覆盖行分析**：[**详细分析所有未覆盖的行**：
  - 行号和代码内容
...
**必须从所有 Collect_Coverage 工具结果中提取并对比**：

### 4.1 覆盖率变化趋势

- **初始覆盖率**：[百分比，如 `88.89%`，从第一次 Collect_Coverage 结果中提取]
- **最终覆盖率**：[百分比，如 `88.89%`，从最后一次 Collect_Coverage 结果中提取]
- **覆盖率变化**：[计算提升或下降的百分比，如 `+0.00%` 或 `-1.11%`]
- **已覆盖行数**：[数量，如 `8`]
- **总行数**：[数量，如 `9`]
- **未覆盖行数**：[数量，如 `1`]
...
**必须列出每次 Collect_Coverage 的结果**，格式如下：

**第 N 次覆盖率收集**（在 SQL 执行前/后）：
- **覆盖率**：XX.XX%
- **已覆盖行数**：X
- **总行数**：X
- **未覆盖行**：[列出所有标记为 `#####` 的行号]
- **gcov_content 关键信息**：[展示 gcov_content 中关键行的执行次数，如 `1: 1850: result = atan2(arg1, arg2);` 或 `#####: 1852: float_overflow_error();`]

### 4.3 未覆盖行详细分析

**必须详细列出所有未覆盖的行**，每个未覆盖行包括：
- **行号**：[如 `1852`]
- **代码内容**：[从 gcov_content 中提取，如 `float_overflow_error();`]
- **分支条件**：[如果是分支语句，说明触发条件，如 `if (unlikely(isinf(result)))`]
- **未覆盖的原因分析**：[至少 100 字，分析为什么这行代码难以覆盖，需要什么输入条件]

### 4.4 覆盖率详情

**详细分析 gcov_content**，说明：
- **执行次数统计**：[哪些行执行了多少次，如 `1: 1850: result = atan2(arg1, arg2);` 表示执行了 1 次]
- **未覆盖行标记**：[哪些行标记为 `#####`（未覆盖），列出所有]
- **执行次数与覆盖情况的关系**：[分析执行次数与覆盖情况的关系，为什么某些行执行了但覆盖率没有提升]
...
**必须从 Coverage Analyzer 和 SQL Tester 的对话中提取所有信息**：
...
**SQL 测试用例 N**：
- **SQL 语句**：[**完整的 SQL 代码**，使用代码块格式，如：
  ```sql
  SELECT datan2(1e308, 1e-308);
  ```]
- **生成时间**：[在哪个 Agent 轮次生成的]
- **目标**：[要覆盖的分支/行号，如 `第 1852 行的 float_overflow_error()`]
- **基于的种子 SQL**：[如果有，说明基于哪个种子 SQL 修改，展示种子 SQL]
- **修改点**：[如果基于种子修改，说明修改了哪些部分，如 `将 atan2d 改为 datan2，将参数从 90 改为 1e308`]
- **预期效果**：[预期这个 SQL 能覆盖哪些行，为什么]
...
**SQL 执行结果 N**：
- **SQL 语句**：[执行的 SQL，使用代码块格式]
- **执行时间**：[在哪个 Agent 轮次执行的]
- **执行状态**：[成功/失败，从 `status` 字段提取]
- **执行结果**：[从 Run_SQL_Test 工具结果中提取的详细结果，包括：
  - `executed`: true/false
  - `message`: 执行消息
  - `db_results`: 数据库返回结果（如果有）
  - `raw_result`: 原始结果]
- **错误信息**：[如果有错误，包含完整的错误信息，从 `error_message` 字段提取]

### 6.3 覆盖率提升分析

**必须详细分析每次 SQL 执行后的覆盖率变化**：

**第 N 次 SQL 执行后的覆盖率分析**：
- **执行前的覆盖率**：[百分比，如 `88.89%`，和未覆盖行列表，如 `[1852]`]
- **执行的 SQL**：[展示 SQL 语句]
- **执行后的覆盖率**：[百分比，如 `88.89%`，和未覆盖行列表，如 `[1852]`]
- **实际提升**：[计算实际提升的百分比和覆盖的行数，如 `+0.00%`，覆盖了 0 行]
- **是否达到预期**：[对比预期和实际结果，如 `未达到预期，第 1852 行仍然未覆盖`]
- **未覆盖行的变化**：[**详细说明**哪些未覆盖行被覆盖了（如果有），哪些仍然未覆盖，为什么]
- **失败原因分析**：[如果覆盖率未提升，分析原因，至少 100 字]

## 7. 工作流程总结（详细）

**必须详细说明整个测试流程，按时间顺序列出每个步骤**：

### 7.1 流程概览

- **总轮数**：[从 agent_history 或 iteration_count 中提取，如 `3 轮`]
- **执行的 Agent**：[列出所有被派发的 Agent，如 `Coverage Analyzer → SQL Tester → Coverage Analyzer`]
- **总耗时**：[如果有时间信息，提取总耗时]

### 7.2 Code Explorer 的工作

**如果 Code Explorer 被派发过，必须详细说明**：

- **派发次数**：[被派发了几次]
- **获取了哪些代码信息**：[列出所有 Get_Code_Context 的调用，包括：
  - 查询的函数/宏名称
  - 返回的文件路径和行号
  - 获取的源代码片段（展示关键部分）]
- **分析了哪些调用关系**：[列出所有 Traverse_Call_Graph 的调用，包括：
  - 遍历的起始函数
  - 调用关系树的关键部分（展示部分树结构）
  - 发现的调用链]
- **提供了哪些关键发现**：[总结 Code Explorer 提供的关键信息，至少 200 字]

### 7.3 Coverage Analyzer 的工作

**必须详细说明 Coverage Analyzer 的所有工作**：

- **收集了几次覆盖率**：[统计 Collect_Coverage 的调用次数，如 `3 次`]
- **每次覆盖率收集的时机和结果**：[列出每次收集的时机（SQL 执行前/后）和结果]
- **查找了几次种子 SQL**：[统计 Search_Nearest_Seed 的调用次数，如 `1 次`]
- **找到的种子 SQL**：[列出所有找到的种子 SQL，展示完整的 SQL 语句]
- **生成了哪些 SQL 测试用例**：[列出所有生成的 SQL，展示完整的 SQL 语句]
- **基于什么信息生成的 SQL**：[说明每次生成 SQL 时基于的信息，如：
  - 基于种子 SQL 修改
  - 基于代码分析（未覆盖行的触发条件）
  - 基于 Code Explorer 提供的代码信息]

### 7.4 SQL Tester 的工作

**必须详细说明 SQL Tester 的所有工作**：

- **执行了哪些 SQL**：[列出所有执行的 SQL，展示完整的 SQL 语句]
- **执行结果如何**：[列出每个 SQL 的执行状态和结果]
- **验证了几次覆盖率**：[统计 Collect_Coverage 的调用次数，如 `2 次`]
- **每次验证的时机和结果**：[列出每次验证的时机（哪个 SQL 执行后）和结果]
...
**第 N 次迭代**：
- **迭代目的**：[这次迭代的目的是什么，如 `尝试覆盖第 1852 行`]
- **执行的步骤**：[列出这次迭代中所有 Agent 的工作]
- **迭代结果**：[这次迭代的结果，覆盖率是否提升]
- **失败原因**：[如果失败，分析原因]
...
- **完成了哪些步骤**：[列出所有完成的步骤，如：
  ...
  - ✅ 验证覆盖率
  - ❌ 覆盖率未提升]
- **哪些步骤成功，哪些失败**：[详细说明每个步骤的成功或失败情况]
- **覆盖率是否提升**：[明确说明覆盖率是否提升，提升了多少，如 `覆盖率未提升，仍为 88.89%`]
- **最终状态**：[说明最终状态，如 `仍有 1 行未覆盖（第 1852 行）`]
...
- **遇到了哪些问题**：[列出所有遇到的问题，如：
  - 问题 1：第 1852 行的 `float_overflow_error()` 无法触发
  - 问题 2：尝试的 SQL 测试用例都无法覆盖该行]
- **问题是如何解决的**：[说明每个问题的解决过程，如果未解决，说明尝试了哪些方法]
- **还有哪些未解决的问题**：[列出所有未解决的问题，每个问题至少 100 字的分析]

### 8.3 下一步行动建议

**必须提供详细的建议**：

- **如果覆盖率未提升，分析原因**：[至少 300 字，详细分析为什么覆盖率未提升，可能的原因包括：
  - 未覆盖行的触发条件过于苛刻
  - SQL 测试用例的参数不正确
  - 需要特殊的输入条件
  - 代码逻辑本身的问题]
- **建议生成哪些新的 SQL 测试用例**：[列出具体的建议，每个建议包括：
  - SQL 语句（展示完整的 SQL）
  - 预期效果（预期覆盖哪些行）
  - 为什么这个 SQL 可能有效]
- **建议的优先级和策略**：[说明建议的优先级，如：
  1. 高优先级：尝试触发 `float_overflow_error()` 的 SQL
  2. 中优先级：分析为什么 `isinf(result)` 条件无法满足
  3. 低优先级：考虑是否需要修改测试策略]

**⚠️ 关键要求（再次强调）**：

1. **必须提取实际数据**：从工具执行结果中提取具体的数值、代码、SQL 语句，不要使用占位符
2. **必须包含代码片段**：不要只说"有分支"，要展示实际的代码，使用代码块格式
3. **必须包含 SQL 语句**：不要只说"生成了 SQL"，要展示完整的 SQL，使用代码块格式
4. **必须详细分析**：不要只写结论，要说明分析过程和依据，每个分析至少 100 字
5. **必须完整准确**：不要遗漏重要信息，不要猜测或简化，所有数据必须从工具结果中提取
6. **必须使用 Markdown 格式**：使用标题、列表、代码块等格式，使报告结构清晰
7. **必须包含所有部分**：不要跳过任何部分，即使某些部分没有数据，也要说明原因

**报告长度要求**：
- 报告应该足够详细，至少 2000 字以上
- 每个主要部分（1-8）至少 200 字
- 包含所有关键信息的具体内容，不要过于简略
- 使用代码块展示所有 SQL 语句和关键代码片段
...
def _should_iterate(messages: List[BaseMessage], agent_history: List[str], max_iterations: int = 3) -> bool:
    ...
    # 检查是否达到最大迭代次数
    # 统计 SQL Tester 被派发的次数（迭代轮次），而不是 SQL 执行次数
    # 因为一次 SQL Tester 调用可能执行多个 SQL
    sql_tester_iteration_count = sum(1 for agent in agent_history if agent == "sql_tester")
    if sql_tester_iteration_count >= max_iterations:
        return False  # 达到最大迭代次数
    
    # 如果覆盖率结果不足2次（coverage_result 为 None），但已执行SQL，应该继续迭代
    if _has_sql_executed(messages) and coverage_result is None:
        return True  # 覆盖率结果不足，需要继续迭代以获取更多覆盖率数据
...
def supervisor_node(state: State) -> dict:
    ...
    # 检查是否超过最大迭代轮数
    if iteration_count >= MAX_ITERATION_ROUNDS:
        print(f"\n[Supervisor] ⚠️ 已达到最大迭代轮数 ({MAX_ITERATION_ROUNDS} 轮)，结束流程", flush=True)
        route = "__end__"
        return {"next_agent": route, "agent_history": agent_history, "iteration_count": iteration_count}
    ...
    # 3.5. Code Explorer 刚完成，应该派发 Coverage Analyzer 基于代码生成精确SQL
    elif (len(agent_history) > 0 and agent_history[-1] == "code_explorer" and
          _has_code_explorer_tool_results(messages)):
        # Code Explorer 刚完成，派发 Coverage Analyzer 基于代码信息生成精确SQL
        route = "coverage_analyzer"
    ...
    print(f"\n[Supervisor] 派发 → {_route_label(route)} (轮数: {iteration_count + 1}/{MAX_ITERATION_ROUNDS})", flush=True)
    # 更新 Agent 派发历史和迭代轮数
    agent_history = state.get("agent_history", [])
    if route != "__end__":
...
**重要**：
- 先用 Get_Code_Context 查函数，用其返回的 file_path、line_number、name 构造 start_func_id
- **格式**：`function:函数名@文件路径:行号`（注意：行号必须是数字，不能是字符串 "line_number"）
- **示例**：
  - Get_Code_Context 返回：`{"name": "datan2", "file_path": "postgresql-17.6/src/backend/utils/adt/float.c", "line_number": 1836}`
  - 正确的 start_func_id：`function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:1836`
  - **错误示例**：`function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:line_number` ❌（不能使用字面字符串 "line_number"）
- file_path 与返回值完全一致（如 postgresql-17.6/src/...），勿改写或省略
- **必须使用实际的行号数字**，不能使用占位符或字符串
...
- **工具调用格式**：
  - Collect_Coverage: {"target_func_id": "function:函数名@文件路径:行号"}
    - **重要**：行号必须是数字，不能是字符串 "line_number"
    - **示例**：`{"target_func_id": "function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:1836"}`
    - **错误示例**：`{"target_func_id": "function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:line_number"}` ❌
  - Search_Nearest_Seed: {"target_func_id": "function:函数名@文件路径:行号"}
    - 同样，行号必须是数字
- **严格禁止使用其他工具**：
...
- Collect_Coverage: {"target_func_id": "function:函数名@文件路径:行号"}
  - **重要**：target_func_id 中的行号必须是数字，不能是字符串 "line_number"
  - **示例**：`{"target_func_id": "function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:1836"}`
  - **错误示例**：`{"target_func_id": "function:datan2@postgresql-17.6/src/backend/utils/adt/float.c:line_number"}` ❌
...
def summary_node(state: State) -> dict:
    ...
    # 使用更多消息上下文（最近100条，确保包含所有工具执行结果和完整对话历史）
    # 对于详细的测试报告，需要包含所有消息以确保不遗漏任何信息
    recent = filtered_messages[-100:] if len(filtered_messages) > 100 else filtered_messages
...
def run_test(
    ...
    # 如果没有指定递归限制，根据最大迭代轮数自动设置
    if recursion_limit is None:
        # 每个轮次可能需要多次递归（派发、执行、验证等），设置为最大轮数的3倍
        recursion_limit = MAX_ITERATION_ROUNDS * 3
...
def get_log_file_path(function_name: str, base_dir: Optional[str] = None) -> str:
    """生成日志文件路径，如果重复则加序号"""
    if base_dir is None:
